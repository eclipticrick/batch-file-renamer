...
const defaultRenameString = renameString(config.default.renameFolders, config.default.renameFiles);
...
const getFirstParamNameWithoutValue = () => config.actions[getState().action].args[getState().args.length];
const getLastParamNameWithValue = () => config.actions[getState().action].args[getState().args.length - 1];
...
const handler = (currentStepNr) => {
    switch (currentStepNr) {
        case 1: return `Specify a folder where you want to rename files (default: '${defaultPath}')`;
        case 2: return `Is this path correct? (y/n) '${statePath()}'`;
        case 3: return `Do you want to rename files or folders or both? (default: ${defaultRenameString})`;
        ...
        case 6: return `The action '${stateAction()}' needs a '${getFirstParamNameWithoutValue()}'`;
        case 7: return `Is '${getLastParamValue()}' the '${getLastParamNameWithValue()}' (y/n)`;
